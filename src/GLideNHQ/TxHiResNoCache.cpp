	...
	, _callback(callback)
{
	...
	strcpy_s(_identc, MAX_PATH, ident);
...
bool TxHiResNoCache::_createFileIndex(bool update)
{
	...
	*/
	if (!update && _callback) {
		_callback("CREATING FILE INDEX. PLEASE WAIT...");
	}
...
{
	/* find it on disk */
	if (!osal_path_exists(directory.c_str())) {
		return false;
	}
	...
	do {
		...
		strcpy_s(entry.fullfname, MAX_PATH, texturefilename.c_str());
		strcpy_s(entry.fname, MAX_PATH, foundfilename);
		...
		if (findFile(chksum64, N64FormatSize(entry.fmt, entry.siz)) != _filesIndex.cend()) {
			 ...
			 */
			DBG_INFO(80, wst("TxNoCache::_createFileIndexInDir: failed to add cksum:%08X %08X file:%s\n"), chksum, palchksum, texturefilename.c_str());
		} else {
			_filesIndex.insert(std::map<uint64, FileIndexEntry>::value_type(chksum64, entry));
			DBG_INFO(80, wst("TxNoCache::_createFileIndexInDir: added cksum:%08X %08X file:%s\n"), chksum, palchksum, texturefilename.c_str());
		}
