...
boolean TxHiResCache::_HiResTexPackPathExists() const
{
	...
	dir_path += _ident;
	return osal_path_exists(dir_path.c_str());
}
...
TxHiResCache::LoadResult TxHiResCache::_loadHiResTextures(const char * dir_path, boolean replace)
{
	DBG_INFO(80, wst("-----\n"));
	DBG_INFO(80, wst("path: %s\n"), dir_path);

	/* find it on disk */
	if (!osal_path_exists(dir_path)) {
	...
	do {
		...
		strcpy(fullfname, texturefilename.c_str());
		strcpy(ident, _ident.c_str());
		strcpy(fname, foundfilename);
		...
		DBG_INFO(80, wst("rom: %s chksum:%08X %08X fmt:%x size:%x\n"), _ident.c_str(), chksum, palchksum, fmt, siz);
		...
		if (added) {
			...
			if (_callback) {
				(*_callback)(wst("[%d] total mem:%.2fmb - %s\n"), int(size()), (totalSize() / 1024) / 1024.0f, fname);
			}
