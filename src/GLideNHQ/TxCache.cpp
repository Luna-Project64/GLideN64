...
public:
	...
	virtual bool save(const char *path, const char *filename, const int config) = 0;
	virtual bool load(const char *path, const char *filename, const int config, bool force) = 0;
...
{
public:
	TxMemoryCache(uint32 _options, const char *cachePath, uint64 cacheLimit, dispInfoFuncExt callback);
	~TxMemoryCache();
	...
	bool save(const char *path, const char *filename, const int config) override;
	bool load(const char *path, const char *filename, const int config, bool force) override;
...
{
	_fullPath = _cachePath + OSAL_DIR_SEPARATOR_STR + _filename;
}
...
bool TxMemoryCache::save(const char *path, const char *filename, int config)
{
	if (_cache.empty())
		return false;
...
bool TxMemoryCache::load(const char *path, const char *filename, int config, bool force)
{
	/* find it on disk */
	assert(_cachePath == path);
	if (_filename.empty()) {
...
{
public:
	TxFileStorage(uint32 _options, const char *cachePath, dispInfoFuncExt callback);
...
bool TxFileStorage::open(bool forRead)
{
	...
	if (osal_path_exists(_fullPath.c_str()) != 0) {
...
{
	if (empty() && osal_path_exists(_fullPath.c_str()) == 0)
		return;
...
bool TxFileStorage::save(const char *path, const char *filename, int config)
{
	assert(_cachePath == path);
	if (_filename.empty()) {
		_filename = filename;
...
bool TxFileStorage::load(const char *path, const char *filename, int config, bool force)
{
	assert(_cachePath == path);
	if (_filename.empty()) {
		_filename = filename;
