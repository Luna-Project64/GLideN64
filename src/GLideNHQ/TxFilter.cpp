	...
	, _txImage(nullptr)
{
   ...
   * a file in the executable directory. */
	...
	if ((_options & FILE_NOTEXCACHE) == FILE_NOTEXCACHE) {
		char fullTexPackPath[MAX_PATH];
		strcpy(fullTexPackPath, texPackPath);
		strcat(fullTexPackPath, OSAL_DIR_SEPARATOR_STR);
		strcat(fullTexPackPath, _ident.c_str());
				...
				ColorFormat gfmt, N64FormatSize n64FmtSz, Checksum r_crc64, boolean isStrongCrc)
{
	...
	if (!_dumpPath.empty() && !_ident.empty()) {
		...
		if (n64FmtSz._format == 0x2) {
			char wbuf[256];
			tx_swprintf(wbuf, 256, wst("/%s#%08X#%01X#%01X#%08X_ciByRGBA.png"), _ident.c_str(), r_crc64._texture, n64FmtSz._format, n64FmtSz._size, r_crc64._palette);
			tmpbuf.append(wbuf);
		} else {
			char wbuf[256];
			tx_swprintf(wbuf, 256, wst("/%s#%08X#%01X#%01X_all.png"), _ident.c_str(), r_crc64._texture, n64FmtSz._format, n64FmtSz._size);
			tmpbuf.append(wbuf);
		}
