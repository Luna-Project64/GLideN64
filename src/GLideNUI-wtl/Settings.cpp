...
void _loadSettings(Config& config, GlSettings& settings)
{
	...
	config.frameBufferEmulation.removeBlackBars = settings.value("removeBlackBars", config.frameBufferEmulation.removeBlackBars).toInt();
	settings.endGroup();
...
void writeSettings(Config& config, const char * _strIniFolder)
{
	...
	if (profile.length() > 0)
	{
		...
		settings.setValue("removeBlackBars", config.frameBufferEmulation.removeBlackBars);
		settings.endGroup();
...
#define WriteCustomSettingS(S) \
	...
	WriteCustomSetting(frameBufferEmulation, removeBlackBars);
	settings.endGroup();
